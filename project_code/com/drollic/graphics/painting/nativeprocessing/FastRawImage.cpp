...
FastRawImage::FastRawImage(int width, int height)
: Width(width), Height(height), xOffset(0), yOffset(0), originalWidth(width), memoryOwner(true), singledata(new PixelData[height * width])
{	
}

FastRawImage::FastRawImage(int width, int height, bool allocateMemory)
: Width(width), Height(height), xOffset(0), yOffset(0), originalWidth(width), memoryOwner(false), singledata(0)
{	
}

/*
FastRawImage::FastRawImage(int width, int height, PixelData* source)
: Width(width), Height(height), singledata(source), xOffset(0), yOffset(0), originalWidth(0), memoryOwner(false)
{	
}
*/

FastRawImage::~FastRawImage()
{
	if (memoryOwner) 
	{
		if (singledata != 0)
...
FastRawImage* FastRawImage::Scale(FastRawImage* input, int width, int height)
{
	FastRawImage* scaledWidth = ScaleWidth(input, width);
	FastRawImage* scaled = ScaleHeight(scaledWidth, height);
	delete scaledWidth;
	return scaled;
}

FastRawImage* FastRawImage::ScaleWidth(FastRawImage* input, int ow)
{
	FastRawImage* output = new FastRawImage(ow, input->Height);
...
FastRawImage* FastRawImage::ScaleHeight(FastRawImage* input, int oh)
{
	FastRawImage* output = new FastRawImage(input->Width, oh);
...
FastRawImage* FastRawImage::Rotate(FastRawImage* input, double thetaInRadians)
{
	double q = thetaInRadians;
	int max = (input->Width > input->Height) ? input->Width : input->Height;
	FastRawImage* output = new FastRawImage(max + max, max + max);
...
void FastRawImage::Rotate(FastRawImage* input, double thetaInRadians, FastRawImage &output)
{
...
void FastRawImage::Crop(int xc, int yc, int w, int h, FastRawImage &result)
{
	int x0, y0;
	...
	result.memoryOwner = false;
}

void FastRawImage::Blend(FastRawImage* inp, FastRawImage* stroke, PixelData rgb, int xc, int yc)
{
