		...
		namespace graphics
		{
			namespace painting
			{
				namespace native					
				{
					...
					{
						class CompositeNativeMomentsPainter : public INativeMomentsPainter
						{
						public:							
							CompositeNativeMomentsPainter(int width, int height, std::vector<FastRawImage*> &originals, std::vector<std::pair<int, int> > &locations, std::vector<FastRawImage*> &strokes, int S);
							~CompositeNativeMomentsPainter();
						...
						protected:
							...
							FastRawImage* CreateCompositePainting();
						};
					}
				}
			}
